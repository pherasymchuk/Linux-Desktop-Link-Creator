import sys
import os
import re
import shutil
import stat  # For chmod
from pathlib import Path # For easier path manipulation
from functools import partial # For connecting signals with arguments

from PySide6.QtWidgets import (
    QApplication, QDialog, QWidget, QVBoxLayout, QHBoxLayout, QGridLayout,
    QPushButton, QLineEdit, QLabel, QComboBox, QPlainTextEdit, QCheckBox,
    QGroupBox, QFileDialog, QMessageBox, QStyleFactory, QFormLayout,
    QSizePolicy, QRadioButton, QButtonGroup, QListWidget, QListWidgetItem,
    QAbstractItemView # For selection mode
)
from PySide6.QtCore import (
    Qt, QStandardPaths, QDir, QFile, QSaveFile, QIODevice, QFileInfo, QProcess,
    QSize, # For icon scaling
    QSettings # <--- ADD THIS
)
from PySide6.QtGui import QIcon, QPixmap, QPainter, QColor

# --- Configuration ---
APP_NAME = "Desktop Link Creator"
ORG_NAME = "YourOrg" # Or leave blank if no settings needed yet
SETTINGS_INTERPRETER_HISTORY = "interpreterHistory"
MAX_INTERPRETER_HISTORY = 15 # Limit history size

# --- Standard Freedesktop Categories (add more if needed) ---
# See: https://specifications.freedesktop.org/menu-spec/latest/apa.html
DESKTOP_CATEGORIES = [
    "AudioVideo", "Audio", "Video", "Development", "Education", "Game",
    "Graphics", "Network", "Office", "Science", "Settings", "System",
    "Utility", "Accessibility", "Documentation", "Core", "GTK", "Qt", "KDE",
    "GNOME", "ConsoleOnly", "WebBrowser"
]
DESKTOP_CATEGORIES.sort() # Keep them alphabetical

# --- Helper Functions ---

def sanitize_filename(name):
    """Creates a safe filename base from the application name."""
    name = name.lower()
    name = re.sub(r'[^\w\s-]', '', name)
    name = re.sub(r'\s+', '-', name).strip('-')
    return name if name else "unnamed-app"

def find_executable_on_path(name):
    """Checks if an executable exists in the system's PATH."""
    return QStandardPaths.findExecutable(name)

def get_best_icon_path(icon_base_name, icon_ext, size=64):
    """Determines the target path within the user's icon directory."""
    base_icon_dir = Path(QStandardPaths.writableLocation(QStandardPaths.StandardLocation.GenericDataLocation)) / "icons" / "hicolor"
    target_name = f"{icon_base_name}{icon_ext}"

    if icon_ext.lower() == ".svg":
        size_dir = "scalable"
    else:
        standard_sizes = [16, 22, 24, 32, 48, 64, 96, 128, 256, 512]
        closest_size = min(standard_sizes, key=lambda s: abs(s - size))
        size_dir = f"{closest_size}x{closest_size}"

    target_dir = base_icon_dir / size_dir / "apps"
    return target_dir, target_name

# --- Main Application Dialog ---

class DesktopLinkerApp(QDialog):
    def __init__(self):
        super().__init__()
        self.settings = QSettings(ORG_NAME, APP_NAME) # Use QSettings
        self.setWindowTitle(APP_NAME)
        self.setMinimumWidth(650)

        # Store paths
        self.script_path = ""
        self.icon_path = ""
        # self.interpreter_path = "" # Now handled by combo box

        self.init_ui()
        self.load_settings() # Load history after UI exists
        self.connect_signals()
        self.apply_stylesheet()
        self.update_interpreter_state() # Set initial state

    def init_ui(self):
        main_layout = QVBoxLayout(self)
        main_layout.setSpacing(15)

        form_layout = QFormLayout()
        form_layout.setRowWrapPolicy(QFormLayout.RowWrapPolicy.WrapLongRows)
        form_layout.setLabelAlignment(Qt.AlignmentFlag.AlignRight)
        form_layout.setHorizontalSpacing(10)
        form_layout.setVerticalSpacing(12)

        # --- Application Name ---
        self.name_edit = QLineEdit()
        self.name_edit.setObjectName("nameEdit")
        self.name_edit.setPlaceholderText("e.g., My Cool App")
        form_layout.addRow("Application &Name:", self.name_edit)

        # --- Script/Program ---
        script_layout = QHBoxLayout()
        self.script_edit = QLineEdit()
        self.script_edit.setObjectName("scriptEdit")
        self.script_edit.setPlaceholderText("Path to the main script or executable")
        self.script_edit.setReadOnly(True)
        self.script_browse_button = QPushButton(QIcon.fromTheme("document-open"), "&Browse...")
        self.script_browse_button.setObjectName("scriptBrowseButton")
        script_layout.addWidget(self.script_edit, 1)
        script_layout.addWidget(self.script_browse_button)
        form_layout.addRow("&Script/Program:", script_layout)

        # --- Execution Method ---
        exec_group = QGroupBox("Execution Method")
        exec_group.setObjectName("execGroup")
        exec_layout = QVBoxLayout(exec_group)
        exec_layout.setSpacing(6)
        self.exec_button_group = QButtonGroup(self)

        self.rb_direct = QRadioButton("&Direct Executable")
        self.rb_direct.setToolTip("Runs the script/program directly (requires execute permission)")
        self.rb_python = QRadioButton("P&ython3 Script")
        self.rb_python.setToolTip("Runs using 'python3' (found in PATH or specified below)")
        self.rb_java = QRadioButton("&Java JAR")
        self.rb_java.setToolTip("Runs using 'java -jar' (found in PATH or specified below)")
        self.rb_bash = QRadioButton("&Bash Script")
        self.rb_bash.setToolTip("Runs using 'bash' (found in PATH or specified below)")
        self.rb_custom = QRadioButton("C&ustom Command Prefix")
        self.rb_custom.setToolTip("Provide the command/interpreter yourself (e.g., /opt/myprog/run)")

        # Assign IDs for easier state management
        self.exec_button_group.addButton(self.rb_direct, 0)
        self.exec_button_group.addButton(self.rb_python, 1)
        self.exec_button_group.addButton(self.rb_java, 2)
        self.exec_button_group.addButton(self.rb_bash, 3)
        self.exec_button_group.addButton(self.rb_custom, 4)

        exec_layout.addWidget(self.rb_direct)
        exec_layout.addWidget(self.rb_python)
        exec_layout.addWidget(self.rb_java)
        exec_layout.addWidget(self.rb_bash)
        exec_layout.addWidget(self.rb_custom)

        self.rb_python.setChecked(True) # Default

        # --- Interpreter/Command Prefix (Using QComboBox) ---
        interp_layout = QHBoxLayout()
        self.interpreter_combo = QComboBox()
        self.interpreter_combo.setObjectName("interpreterCombo")
        self.interpreter_combo.setEditable(True)
        self.interpreter_combo.setInsertPolicy(QComboBox.InsertPolicy.NoInsert) # Don't automatically add typed text
        self.interpreter_combo.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
        self.interpreter_combo.setToolTip("Specify path or command (optional for Python/Java/Bash - uses PATH if blank)")
        # Add placeholder text behavior via focus events or a custom completer if needed
        # self.interpreter_combo.lineEdit().setPlaceholderText("e.g., /usr/bin/python3.10") # Set on lineEdit()

        self.interpreter_browse_button = QPushButton(QIcon.fromTheme("document-open"), "B&rowse...")
        self.interpreter_browse_button.setObjectName("interpreterBrowseButton")
        interp_layout.addWidget(self.interpreter_combo, 1)
        interp_layout.addWidget(self.interpreter_browse_button)

        self.interpreter_label = QLabel("(Recommended) Interpreter/Command Prefix:") # Create label separately
        exec_layout.addSpacing(10)
        exec_layout.addWidget(self.interpreter_label)
        exec_layout.addLayout(interp_layout)

        form_layout.addRow(exec_group)

        # --- Icon ---
        icon_row_layout = QHBoxLayout() # Layout for the whole row
        icon_input_layout = QHBoxLayout() # Layout for edit + button

        self.icon_edit = QLineEdit()
        self.icon_edit.setObjectName("iconEdit")
        self.icon_edit.setPlaceholderText("Path to .png or .svg icon file")
        self.icon_edit.setReadOnly(True)
        self.icon_browse_button = QPushButton(QIcon.fromTheme("preferences-setIcon", QIcon.fromTheme("image-x-generic")), "&Icon...") # More specific icon + fallback
        self.icon_browse_button.setObjectName("iconBrowseButton")
        icon_input_layout.addWidget(self.icon_edit, 1)
        icon_input_layout.addWidget(self.icon_browse_button)

        self.icon_preview_label = QLabel()
        self.icon_preview_label.setObjectName("iconPreviewLabel")
        self.icon_preview_label.setFixedSize(48, 48) # Size for preview
        self.icon_preview_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.icon_preview_label.setFrameShape(QLabel.Shape.StyledPanel) # Add border
        self.icon_preview_label.setToolTip("Selected icon preview")

        icon_row_layout.addLayout(icon_input_layout, 1) # Input takes most space
        icon_row_layout.addWidget(self.icon_preview_label) # Add preview at the end

        form_layout.addRow("&Icon File:", icon_row_layout)

        # --- Options ---
        options_group = QGroupBox("Optional Details")
        options_group.setObjectName("optionsGroup")
        options_layout = QFormLayout(options_group)
        options_layout.setRowWrapPolicy(QFormLayout.RowWrapPolicy.WrapLongRows)
        options_layout.setLabelAlignment(Qt.AlignmentFlag.AlignRight)
        options_layout.setHorizontalSpacing(10)
        options_layout.setVerticalSpacing(12)

        self.terminal_checkbox = QCheckBox("Run in Terminal")
        self.terminal_checkbox.setToolTip("Check this if the application requires a terminal window (e.g., console output)")
        options_layout.addRow(self.terminal_checkbox)

        self.comment_edit = QLineEdit()
        self.comment_edit.setObjectName("commentEdit")
        self.comment_edit.setPlaceholderText("e.g., A utility to manage files")
        options_layout.addRow("C&omment:", self.comment_edit)

        # --- Categories List ---
        self.categories_list = QListWidget()
        self.categories_list.setObjectName("categoriesList")
        self.categories_list.setSelectionMode(QAbstractItemView.SelectionMode.ExtendedSelection)
        self.categories_list.setToolTip("Select one or more categories (Ctrl+Click or Shift+Click)")
        self.categories_list.setMinimumHeight(120)
        self.categories_list.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
        for category in DESKTOP_CATEGORIES:
            self.categories_list.addItem(QListWidgetItem(category))

        options_layout.addRow(QLabel("Ca&tegories:"), self.categories_list)

        form_layout.addRow(options_group)

        main_layout.addLayout(form_layout)
        main_layout.addStretch(1)

        # --- Bottom Buttons (Generate & Clear) ---
        button_layout = QHBoxLayout()
        button_layout.addStretch(1)
        self.clear_button = QPushButton(QIcon.fromTheme("edit-clear"), "&Clear All")
        self.clear_button.setObjectName("clearButton")
        self.generate_button = QPushButton(QIcon.fromTheme("document-save"), "&Generate .desktop File")
        self.generate_button.setObjectName("generateButton")
        # Make Generate button visually more prominent if desired
        # self.generate_button.setProperty("primary", True) # For custom styling

        button_layout.addWidget(self.clear_button)
        button_layout.addSpacing(20) # Space between buttons
        button_layout.addWidget(self.generate_button)
        button_layout.addStretch(1)
        main_layout.addLayout(button_layout)


    def apply_stylesheet(self):
        # More colorful theme with better contrast
        primary_color = "#007AD9" # Vivid Blue
        secondary_color = "#F5F5F5" # Very Light Gray Background
        accent_color = "#1ED760" # Spotify-like Green Accent
        text_color = "#222222" # Dark Gray Text
        border_color = "#D1D1D1" # Light Gray Borders
        button_hover_bg = "#0088F0"
        button_pressed_bg = "#006BBF"
        group_bg = "#FFFFFF" # White Group Background
        group_title_color = "#005AAA" # Darker Blue Title
        input_bg = "#FFFFFF"
        selection_color = primary_color
        selection_text_color = "#FFFFFF"

        self.setStyleSheet(f"""
            QDialog {{
                background-color: {secondary_color};
                font-size: 11pt; /* Base font size */
            }}
            QGroupBox {{
                font-weight: bold;
                border: 1px solid {border_color};
                border-radius: 6px;
                margin-top: 0.7em;
                padding: 12px 12px 15px 12px; /* More padding bottom */
                background-color: {group_bg};
            }}
            QGroupBox::title {{
                subcontrol-origin: margin;
                subcontrol-position: top left;
                padding: 3px 8px; /* More padding for title */
                left: 12px;
                color: {group_title_color};
                background-color: {secondary_color};
                border: 1px solid {border_color};
                border-bottom: none; /* Blend with top border */
                border-top-left-radius: 5px;
                border-top-right-radius: 5px;
            }}
            QPushButton {{
                padding: 8px 16px; /* Larger padding */
                border: 1px solid {primary_color};
                border-radius: 5px;
                background-color: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 {primary_color}, stop:1 {button_pressed_bg});
                color: white;
                font-weight: bold;
                min-width: 100px; /* Wider buttons */
            }}
            QPushButton:hover {{
                background-color: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 {button_hover_bg}, stop:1 {primary_color});
                border-color: {button_pressed_bg};
            }}
            QPushButton:pressed {{
                background-color: {button_pressed_bg};
                border-color: {group_title_color};
            }}
            QPushButton:disabled {{
                 background-color: #B0BEC5; /* Gray out disabled buttons */
                 border-color: #90A4AE;
                 color: #607D8B;
             }}

            /* Style specific buttons */
            QPushButton#scriptBrowseButton, QPushButton#iconBrowseButton,
            QPushButton#interpreterBrowseButton, QPushButton#clearButton {{
                 min-width: 80px;
                 padding: 7px 10px;
                 font-weight: normal;
                 background-color: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 #FFFFFF, stop:1 #E0E0E0);
                 border: 1px solid #BDBDBD;
                 color: {text_color};
            }}
             QPushButton#scriptBrowseButton:hover, QPushButton#iconBrowseButton:hover,
             QPushButton#interpreterBrowseButton:hover, QPushButton#clearButton:hover {{
                  background-color: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 #F5F5F5, stop:1 #D5D5D5);
                  border-color: #9E9E9E;
              }}
             QPushButton#scriptBrowseButton:pressed, QPushButton#iconBrowseButton:pressed,
             QPushButton#interpreterBrowseButton:pressed, QPushButton#clearButton:pressed {{
                  background-color: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 #D5D5D5, stop:1 #F5F5F5);
              }}

            QLineEdit, QComboBox {{
                padding: 6px; /* More padding */
                border: 1px solid {border_color};
                border-radius: 4px;
                background-color: {input_bg};
                color: {text_color};
            }}
             QLineEdit:read-only {{
                 background-color: #E0E0E0; /* Grayer read-only */
                 color: #757575;
             }}
            QComboBox::drop-down {{
                /* Style dropdown arrow if needed */
                border-left: 1px solid {border_color};
                padding-right: 5px;
             }}
            QComboBox QAbstractItemView {{ /* Dropdown list style */
                border: 1px solid {border_color};
                background-color: {input_bg};
                selection-background-color: {selection_color};
                selection-color: {selection_text_color};
                color: {text_color};
            }}
            QFormLayout QLabel {{
                font-weight: bold;
                color: {group_title_color};
                padding-top: 5px; /* Align better with taller inputs */
             }}
            QCheckBox, QRadioButton {{
                 spacing: 6px; /* Space between box/radio and text */
                 color: {text_color}; /* Ensure text color */
             }}
            QListWidget {{
                border: 1px solid {border_color};
                border-radius: 4px;
                background-color: {input_bg};
                color: {text_color};
            }}
            QListWidget::item:selected {{
                background-color: {selection_color};
                color: {selection_text_color};
            }}
            QListWidget::item:hover {{
                background-color: #E3F2FD; /* Light Blue hover */
                color: {text_color};
            }}
             QLabel#iconPreviewLabel {{ /* Style icon preview */
                 background-color: #ECEFF1;
                 border: 1px dashed #B0BEC5; /* Dashed border */
                 color: #78909C; /* Placeholder text color */
             }}
        """)


    def connect_signals(self):
        self.script_browse_button.clicked.connect(self.browse_script)
        self.icon_browse_button.clicked.connect(self.browse_icon)
        self.interpreter_browse_button.clicked.connect(self.browse_interpreter)
        self.generate_button.clicked.connect(self.generate_desktop_file)
        self.clear_button.clicked.connect(self.clear_fields)

        # Update interpreter field state when radio button changes
        # Use partial to pass the button id to the slot
        for button_id in self.exec_button_group.buttons():
             button_id.clicked.connect(self.update_interpreter_state)
             # Or use: self.exec_button_group.buttonClicked.connect(self.update_interpreter_state) if no args needed

        # Update icon preview when icon path changes (via browse)
        # self.icon_edit.textChanged.connect(self.update_icon_preview) # Connect if needed, browse_icon handles it now

    # --- Settings ---

    def load_settings(self):
        # Load interpreter history
        history = self.settings.value(SETTINGS_INTERPRETER_HISTORY, [])
        if isinstance(history, str): history = [history] # Handle case where only one item was saved
        if history:
            self.interpreter_combo.addItems(history)

    def save_settings(self):
        # Save interpreter history (items currently in the combo box)
        history = []
        for i in range(self.interpreter_combo.count()):
             item_text = self.interpreter_combo.itemText(i)
             if item_text: # Avoid saving empty strings if any crept in
                history.append(item_text)
        # Limit history size before saving
        if len(history) > MAX_INTERPRETER_HISTORY:
            history = history[:MAX_INTERPRETER_HISTORY]
        self.settings.setValue(SETTINGS_INTERPRETER_HISTORY, history)

    def closeEvent(self, event):
        """Save settings on close."""
        self.save_settings()
        super().closeEvent(event)

    # --- UI State Update ---

# --- Inside DesktopLinkerApp class ---

    def update_interpreter_state(self):
        """Enable/disable interpreter combo based on selected execution method."""
        checked_id = self.exec_button_group.checkedId()
        enable = checked_id in [1, 2, 3, 4]
        self.interpreter_label.setEnabled(enable)
        self.interpreter_combo.setEnabled(enable)
        self.interpreter_browse_button.setEnabled(enable)

        # Update tooltip based on state
        if checked_id == 0: # Direct
             self.interpreter_combo.setToolTip("Not applicable for direct execution.")
        elif checked_id == 4: # Custom
             self.interpreter_combo.setToolTip("Enter the full command or path to the executable.\nIt's best to use the full path if possible.")
        else: # Python, Java, Bash
             default_name = self._get_default_interpreter_name(checked_id)
             # --- MODIFIED TOOLTIP ---
             self.interpreter_combo.setToolTip(f"Recommended: Specify full path via Browse... \n"
                                               f"If left blank, attempts to use '{default_name}' from system PATH (less reliable).")
             # --- END MODIFIED TOOLTIP ---


    def _get_default_interpreter_name(self, method_id):
        """Helper to get default interpreter name for placeholder."""
        if method_id == 1: return "python3"
        if method_id == 2: return "java"
        if method_id == 3: return "bash"
        return ""

    # --- Browser Slots ---

    def browse_script(self):
        start_dir = str(Path.home())
        if self.script_path and Path(self.script_path).parent.exists():
             start_dir = str(Path(self.script_path).parent)

        path, _ = QFileDialog.getOpenFileName(self, "Select Script or Program", start_dir, "All Files (*)")
        if path:
            if not Path(path).is_file():
                 QMessageBox.warning(self, "Invalid Selection", "Selected path is not a file.")
                 return
            native_path = QDir.toNativeSeparators(path)
            self.script_path = native_path
            self.script_edit.setText(native_path)
            if not self.name_edit.text():
                base_name = QFileInfo(path).completeBaseName().replace('_', ' ').replace('-', ' ').title()
                self.name_edit.setText(base_name)

    def browse_icon(self):
        start_dir = str(Path.home())
        if self.icon_path and Path(self.icon_path).parent.exists():
             start_dir = str(Path(self.icon_path).parent)

        path, _ = QFileDialog.getOpenFileName(self, "Select Icon File", start_dir, "Images (*.png *.svg *.xpm);;All Files (*)")
        if path:
             if not Path(path).is_file():
                 QMessageBox.warning(self, "Invalid Selection", "Selected path is not a file.")
                 return
             native_path = QDir.toNativeSeparators(path)
             self.icon_path = native_path
             self.icon_edit.setText(native_path)
             self.update_icon_preview(native_path) # Update preview

    def browse_interpreter(self):
        start_dir = "/usr/bin"
        current_interp = self.interpreter_combo.currentText()
        if current_interp and Path(current_interp).parent.exists():
             start_dir = str(Path(current_interp).parent)
        elif self.script_path and Path(self.script_path).parent.exists():
             start_dir = str(Path(self.script_path).parent)

        path, _ = QFileDialog.getOpenFileName(self, "Select Interpreter or Command", start_dir, "All Files (*)")
        if path:
             if not Path(path).is_file(): # Could be a command not in PATH yet
                  # Allow selecting non-files if user intends a command name
                  pass
             native_path = QDir.toNativeSeparators(path)
             # Set the text in the editable combo box
             self.interpreter_combo.setCurrentText(native_path)
             # No need to store separately, read from combo box later

    # --- Icon Preview ---
    def update_icon_preview(self, icon_path):
        if icon_path and Path(icon_path).is_file():
            pixmap = QPixmap(icon_path)
            if not pixmap.isNull():
                 # Scale pixmap smoothly while preserving aspect ratio to fit the label
                 scaled_pixmap = pixmap.scaled(self.icon_preview_label.size(),
                                               Qt.AspectRatioMode.KeepAspectRatio,
                                               Qt.TransformationMode.SmoothTransformation)
                 self.icon_preview_label.setPixmap(scaled_pixmap)
                 return
        # Clear preview if path is invalid or pixmap is null
        self.icon_preview_label.clear()
        self.icon_preview_label.setText("?") # Indicate no preview

    # --- History Management ---
    def add_interpreter_to_history(self, interpreter_cmd):
        """Adds a command to the history combo box if it's not already there."""
        if not interpreter_cmd: return # Don't add empty strings

        # Check if item already exists (case-sensitive check)
        if self.interpreter_combo.findText(interpreter_cmd, Qt.MatchFlag.MatchFixedString) == -1:
            # Insert at the top (index 0)
            self.interpreter_combo.insertItem(0, interpreter_cmd)
            # Limit history size
            while self.interpreter_combo.count() > MAX_INTERPRETER_HISTORY:
                 self.interpreter_combo.removeItem(self.interpreter_combo.count() - 1) # Remove oldest (last item)
            # Optionally select the newly added item
            self.interpreter_combo.setCurrentIndex(0)


    # --- Generation Logic ---

# --- Inside DesktopLinkerApp class ---

    # --- Generation Logic ---

    def generate_desktop_file(self):
        app_name = self.name_edit.text().strip()
        script_source_path_str = self.script_path # Original path selected by user
        icon_source_path_str = self.icon_path
        interpreter_prefix = self.interpreter_combo.currentText().strip()
        run_in_terminal = self.terminal_checkbox.isChecked()
        comment = self.comment_edit.text().strip()
        exec_method_id = self.exec_button_group.checkedId()

        selected_items = self.categories_list.selectedItems()
        selected_categories = [item.text() for item in selected_items]
        categories_str = ";".join(selected_categories)
        if categories_str and not categories_str.endswith(';'):
            categories_str += ';'

        # --- Input Validation ---
        if not app_name:
            QMessageBox.warning(self, "Input Missing", "Please enter an Application Name.")
            return
        # Validate the SOURCE script path
        if not script_source_path_str or not Path(script_source_path_str).is_file():
            QMessageBox.warning(self, "Input Missing", "Please select a valid SOURCE Script or Program file.")
            return
        if not icon_source_path_str or not Path(icon_source_path_str).is_file():
            QMessageBox.warning(self, "Input Missing", "Please select a valid Icon file.")
            return

        # --- Prepare Paths and Names ---
        script_source_path = Path(script_source_path_str)
        icon_source_path = Path(icon_source_path_str)
        icon_ext = icon_source_path.suffix.lower()
        if icon_ext not in [".png", ".svg", ".xpm"]:
            QMessageBox.warning(self, "Invalid Icon", "Please select a PNG, SVG, or XPM icon file.")
            return

        base_filename = sanitize_filename(app_name)
        # icon_name_base = base_filename # No longer needed if using full path for Icon=

        desktop_filename = f"{base_filename}.desktop"
        app_dir_path = Path(QStandardPaths.writableLocation(QStandardPaths.StandardLocation.ApplicationsLocation))
        desktop_file_path = app_dir_path / desktop_filename

        # --- NEW: Determine Script Target Path ---
        # Create a subdirectory in ~/.local/bin based on the sanitized app name
        script_target_base_dir = Path(QStandardPaths.writableLocation(QStandardPaths.StandardLocation.HomeLocation)) / ".local" / "bin"
        script_target_subdir = script_target_base_dir / base_filename
        # Use the original script's filename for the copied file
        script_target_path = script_target_subdir / script_source_path.name

        # --- Determine Icon Target Path (using full path approach as requested) ---
        icon_target_filename = f"{base_filename}{icon_ext}" # Use sanitized name + original extension
        # Place directly in ~/.local/share/icons (still less standard than hicolor)
        icons_base_dir = Path(QStandardPaths.writableLocation(QStandardPaths.StandardLocation.GenericDataLocation)) / "icons"
        icon_target_path = icons_base_dir / icon_target_filename


        # --- Determine Exec Command (using the NEW script_target_path) ---
        exec_command = ""
        interpreter_cmd = interpreter_prefix

        if not interpreter_cmd and exec_method_id in [1, 2, 3]:
            if exec_method_id == 1: default_name = "python3"
            elif exec_method_id == 2: default_name = "java"
            elif exec_method_id == 3: default_name = "bash"
            interpreter_cmd = find_executable_on_path(default_name) or default_name

        should_add_to_history = (exec_method_id == 4 and interpreter_cmd) or \
                                (exec_method_id in [1,2,3] and interpreter_prefix)

        # --- Use the COPIED script path in the Exec command ---
        quoted_script_target_path = f'"{str(script_target_path)}"' # Ensure it's a string

        if exec_method_id == 0: # Direct
            exec_command = quoted_script_target_path
        elif exec_method_id == 1: # Python
            exec_command = f"{interpreter_cmd} {quoted_script_target_path}"
        elif exec_method_id == 2: # Java JAR
            exec_command = f"{interpreter_cmd} -jar {quoted_script_target_path}"
        elif exec_method_id == 3: # Bash
             exec_command = f"{interpreter_cmd} {quoted_script_target_path}"
        elif exec_method_id == 4: # Custom
             if interpreter_cmd:
                 exec_command = f"{interpreter_cmd} {quoted_script_target_path}"
             else:
                 exec_command = quoted_script_target_path


        # --- Generate .desktop Content (using FULL path for Icon) ---
        desktop_content = f"""[Desktop Entry]
Version=1.1
Type=Application
Name={app_name}
Exec={exec_command}
Icon={str(icon_target_path)}
Terminal={'true' if run_in_terminal else 'false'}
"""
        if comment:
            desktop_content += f"Comment={comment}\n"
        if categories_str:
            desktop_content += f"Categories={categories_str}\n"
        # Use the directory of the COPIED script for the Path entry
        desktop_content += f"Path={str(script_target_path.parent)}\n"
        desktop_content += f"StartupNotify=true\n"


        # --- Perform File Operations ---
        try:
            # --- NEW: Create script target directory ---
            script_target_subdir.mkdir(parents=True, exist_ok=True)
            self.log_output(f"Ensured script directory exists: {script_target_subdir}")

            # --- NEW: Copy the script ---
            shutil.copy2(str(script_source_path), str(script_target_path))
            self.log_output(f"Script copied from '{script_source_path}' to '{script_target_path}'")

            # 1. Set executable bit on the COPIED script
            current_stat = os.stat(script_target_path)
            new_mode = current_stat.st_mode | stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH
            if current_stat.st_mode != new_mode:
                 os.chmod(script_target_path, new_mode)
                 self.log_output(f"Set execute permission on: {script_target_path}")
            else:
                 self.log_output(f"Execute permission already set on: {script_target_path}")

            # 2. Create icon directory if needed
            icons_base_dir.mkdir(parents=True, exist_ok=True) # Ensure ~/.local/share/icons exists

            # 3. Copy icon file (using the direct path)
            shutil.copy2(str(icon_source_path), str(icon_target_path))
            self.log_output(f"Icon copied to: {icon_target_path}")

            # 4. Create .desktop directory
            app_dir_path.mkdir(parents=True, exist_ok=True)

            # 5. Write .desktop file safely
            save_file = QSaveFile(str(desktop_file_path))
            if not save_file.open(QIODevice.OpenModeFlag.WriteOnly | QIODevice.OpenModeFlag.Text):
                raise OSError(f"Could not open QSaveFile for writing: {save_file.errorString()}")
            if save_file.write(desktop_content.encode('utf-8')) == -1:
                 raise OSError(f"Could not write to QSaveFile: {save_file.errorString()}")
            if not save_file.commit():
                raise OSError(f"Could not commit QSaveFile: {save_file.errorString()}")

            self.log_output(f".desktop file created: {desktop_file_path}")

            # 6. Add interpreter to history if needed
            if should_add_to_history:
                self.add_interpreter_to_history(interpreter_cmd)

            # 7. Success Message (references the final script location now)
            if find_executable_on_path("update-desktop-database"):
                reply = QMessageBox.information(self, "Success",
                                                f"Successfully created shortcut for '{app_name}'!\n"
                                                f"Script copied to: {script_target_path}\n"
                                                f".desktop file: {desktop_file_path}\n"
                                                f"Icon: {icon_target_path}\n\n"
                                                "You may need to run 'update-desktop-database' or log out/in for the shortcut to appear in application menus immediately.",
                                                QMessageBox.StandardButton.Ok)
            else:
                 QMessageBox.information(self, "Success",
                                        f"Successfully created shortcut for '{app_name}'!\n"
                                        f"Script copied to: {script_target_path}\n"
                                        f".desktop file: {desktop_file_path}\n"
                                        f"Icon: {icon_target_path}\n\n"
                                        "Log out and log back in for the shortcut to appear in application menus.")

            # self.clear_fields()

        except Exception as e:
            self.log_output(f"Error: {e}", error=True)
            # Consider adding more specific error handling (e.g., for permission errors during copy/chmod)
            QMessageBox.critical(self, "Error", f"An error occurred:\n{e}")

    # ... (rest of the class remains the same - log_output, clear_fields, etc.) ...

# --- Main Execution (No changes needed here) ---
# if __name__ == "__main__":
#     ...

    def log_output(self, message, error=False):
        prefix = "ERROR: " if error else "INFO: "
        print(f"{prefix}{message}")

    # --- Clear Fields ---
    def clear_fields(self):
         self.name_edit.clear()
         self.script_edit.clear()
         self.script_path = ""
         self.icon_edit.clear()
         self.icon_path = ""
         self.icon_preview_label.clear() # Clear preview
         self.icon_preview_label.setText("?")
         self.interpreter_combo.setCurrentText("") # Clear current text
         # Keep history items in the combo box, don't clear them here
         self.terminal_checkbox.setChecked(False)
         self.comment_edit.clear()
         self.categories_list.clearSelection() # Clear selected categories
         self.rb_python.setChecked(True) # Reset to default radio
         self.update_interpreter_state() # Update enabled state


# --- Main Execution ---
if __name__ == "__main__":
    app = QApplication(sys.argv)

    # Try to set a nice default style
    if "Fusion" in QStyleFactory.keys():
         app.setStyle(QStyleFactory.create("Fusion"))

    dialog = DesktopLinkerApp()
    dialog.show()
    sys.exit(app.exec())
